...
function waitForClinicResult(expectedLabel,timeoutMs=18000){
  ...
  return new Promise((resolve,reject) => {
    ...
    const check = () => {
      ...
      if(snapshot && (actual === expected || actual.includes(expected) || expected.includes(actual))){
        setTimeout(() => resolve(snapshot),250);
        return;
      }
      if(Date.now() - startedAt >= timeoutMs){
        reject(new Error(`Tempo esgotado ao consultar ${expectedLabel}.`));
        return;
      }
...
async function scanClinics(requestId){
  ...
  try{
    ...
    notify({type:"CENSUS_RESULT",requestId,payload:{sectors:results,capturedAt}});
    setWidgetText(`SCIR atualizado: ${results.length} setores`);
  }catch(error){
