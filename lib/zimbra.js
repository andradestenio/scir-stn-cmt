...
const MINIMUM_SYNC_INTERVAL_MS = 25_000;
...
export async function syncZimbraNotifications({force=false}={}){
  const row = await getEmailIntegration();
  ...
  if(!force && Date.now() - lastChecked < MINIMUM_SYNC_INTERVAL_MS){
  ...
  try{
    ...
    const connection = await openMailbox(password);
